# HostelApp — Multi-Room, Multi-User Hostel Task Management
...
- **`backend/`** — a Node.js + Express API with its own data storage.
  This has to be running for the app to work at all.
- **`frontend/hostelapp.html`** — a single-file web app that talks to that
  backend over HTTP. Open it in a browser once the backend is running.
...
2. Open a terminal in the `backend` folder and run:
   ...
   npm start
   ```
   You should see: `HostelApp backend listening on http://localhost:4000`
3. Open `frontend/hostelapp.html` directly in a browser (double-click it, or
   drag it into a browser window). It's hardcoded to talk to
   `http://localhost:4000` — leave the backend terminal running.
